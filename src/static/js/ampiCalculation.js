function calculatePoints() {
    ...
    // Auto Percepção
    const self_perception = document.querySelector(
        'input[name="auto-percepcao"]:checked',
    ).value;
    if (self_perception != "Boa") {
        points += 1;
    }

    // Suporte social
    const support = document.querySelector(
        'input[name="suporte-social"]:checked',
    );
    ...
    // Condições crônicas
    const chronicConditions = document.querySelectorAll(
        'input[name="condicoes-cronicas"]:checked',
    );
    ...
    // Internações
    const hospitalizations = parseInt(
        document.getElementById("internacoes").value,
    );
    ...
    // Limitação física
    const physicalLimitation = document.querySelector(
        'input[name="limitacao-fisica"]:checked',
    );
    ...
    const dailyActivities = document.querySelector(
        'input[name="atividades-basicas"]:checked',
    );
    ...
    const instrumentalActivities = document.querySelector(
        'input[name="atividades-instrumentais"]:checked',
    );
    ...
    // Incontinência
    const incontinence = document.querySelector(
        'input[name="incontinencia"]:checked',
    );
    ...
    const weightLoss = document.querySelector(
        'input[name="perda-peso"]:checked',
    );
    ...
    // Abandono
    const abandonment = document.querySelector(
        'input[name="abandono"]:checked',
    );
